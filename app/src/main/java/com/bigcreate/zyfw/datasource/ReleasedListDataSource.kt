...
class ReleasedListDataSource(private val networkState: MutableLiveData<NetworkState>): PageKeyedDataSource<Int, Project>() {
	private val request = PageRequest(Attributes.token,1)
	override fun loadAfter(params: PageKeyedDataSource.LoadParams<Int>, callback: PageKeyedDataSource.LoadCallback<Int, Project>) {
		tryLoad(params,callback,params.key - 1)
	}

	override fun loadBefore(params: PageKeyedDataSource.LoadParams<Int>, callback: PageKeyedDataSource.LoadCallback<Int, Project>) {
		tryLoad(params,callback,params.key + 1)
	}

	override fun loadInitial(params: PageKeyedDataSource.LoadInitialParams<Int>, callback: PageKeyedDataSource.LoadInitialCallback<Int, Project>) {
		request.token = Attributes.token
		networkState.postValue(NetworkState.LOADING)
		RemoteService.getUserReleasedList(request).enqueue(object : Callback<JsonObject> {
	...
	fun tryLoad(params: PageKeyedDataSource.LoadParams<Int>, callback: PageKeyedDataSource.LoadCallback<Int, Project>, nextKey: Int?) {
		request.token = Attributes.token
